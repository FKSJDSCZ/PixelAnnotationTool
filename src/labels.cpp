...
LabelInfo::LabelInfo(QString name, QString category, int id, int id_category, QColor color)
{
    this->name = name;
...
void LabelInfo::read(const QJsonObject& json)
{
    ...
    id_category = json["id_categorie"].toInt();
...
void LabelInfo::write(QJsonObject& json) const
{
    json["id"] = id;
    json["name"] = name;
    json["categorie"] = category;
    json["id_categorie"] = id_category;
    QJsonArray jarray;
